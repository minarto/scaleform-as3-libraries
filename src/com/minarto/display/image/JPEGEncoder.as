package com.minarto.display.image
{
	...
	public final class JPEGEncoder
	{
		...
		private function initQuantTables(sf:int):void
		{
			...
			while(row < 8)
			{
				...
				while(col < 8)
				{
					r = aasf[row] * aasf[col] * 8;
					
					fdtbl_Y[k]  = (1 / (YTable [ZigZag[k]] * r));
					fdtbl_UV[k] = (1 / (UVTable[ZigZag[k++]] * r));
		...
		private function writeBits(bs:BitString):void
		{
			...
			var writeByteF:Function = byteout.writeByte;
			while ( posval >= 0 )
			{
				if (value & (1 << posval) )
					bytenew |= (1 << bytepos);
		...
		private function fDCTQuant(data:Vector.<Number>, fdtbl:Vector.<Number>):Vector.<Number>
		{
			...
			var k:int = 0;
			var roundF:Function = Math.round;
			while(k < 64)
			{
				// Apply the quantization and scaling factor & Round to nearest integer
				data[k] = roundF(data[k]*fdtbl[k]);
		...
		private function writeDHT():void
		{
			...
			writeByteF(0); // HTYDCinfo
			var i:int;
			while(i < 16)
			{
				writeByteF(std_dc_luminance_nrcodes[i+1]);
			...
			while(i <= 11)
			{
				writeByteF(std_dc_luminance_values[i]);
				
				++ i;
			}
			
			writeByteF(0x10); // HTYACinfo
			
			i = 0;
			while(i < 16)
			{
				writeByteF(std_ac_luminance_nrcodes[i+1]);
			...
			while(i <= 161)
			{
				writeByteF(std_ac_luminance_values[i]);
				
				++ i;
			}
			
			writeByteF(1); // HTUDCinfo
			
			i = 0;
			while(i < 16)
			{
				writeByteF(std_dc_chrominance_nrcodes[i+1]);
			...
			while(i <= 11)
			{
				writeByteF(std_dc_chrominance_values[i]);
				
				++ i;
			}
			
			writeByteF(0x11); // HTUACinfo
			
			i = 0;
			while(i < 16)
			{
				writeByteF(std_ac_chrominance_nrcodes[i+1]);
			...
			while(i <= 161)
			{
				writeByteF(std_ac_chrominance_values[i]);
		...
		private function processDU(CDU:Vector.<Number>, fdtbl:Vector.<Number>, DC:Number, HTDC:Vector.<BitString>, HTAC:Vector.<BitString>):Number
		{
			...
			if (Diff==0) {
				...
				writeBits(HTDC[category[int(32767+Diff)]]);
		...
		private function RGB2YUV(img:BitmapData, xpos:int, ypos:int):void
		{
			...
			var x:int;
			var y:int=0;
			var getPixel32F:Function = img.getPixel32;
			while(y < 8)
			{
				x = 0;
				while(x < 8)
				{
					var P:uint = getPixel32F(xpos+x,ypos+y);
					var R:int = (P>>16)&0xFF;
					var G:int = (P>> 8)&0xFF;
					var B:int = (P    )&0xFF;
		...
		public function JPEGEncoder(quality:int=100)
		{
			if (quality <= 0) {
				quality = 1;
			}
